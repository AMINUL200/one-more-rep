...
const MangeOrders = () => {
  ...
  const fetchOrders = async () => {
    setLoading(true);
    try {
      const response = await api.get("/admin/accounts-orders");
  ...
  const exportToExcel = () => {
    ...
    try {
      const exportData = filteredOrders.map(order => ({
        ...
        'Customer Name': order.receiver_name || order.customer_name || 'N/A',
        'Email': order.email || 'N/A',
        'Phone': order.receiver_phone || order.phone || 'N/A',
        ...
        'Payment Status': order.payment_status?.charAt(0).toUpperCase() + order.payment_status?.slice(1) || 'N/A',
        'Order Status': order.order_status?.charAt(0).toUpperCase() + order.order_status?.slice(1) || 'N/A',
        'Payment Method': order.payment_method || 'N/A',
        'Address': `${order.address || ''} ${order.city || ''} ${order.state || ''} ${order.pincode || ''}`.trim() || 'N/A'
      }));
      ...
      const date = new Date();
      const fileName = `orders_${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}.xlsx`;
  ...
  return (
    <>
      ...
      {/* Order Details Modal (same as original) */}
      <AnimatePresence>
            ...
            >
              <div className="min-h-screen px-4 flex items-center justify-center">
                ...
                >
                  ...
                  {/* Modal Body - same content as original */}
