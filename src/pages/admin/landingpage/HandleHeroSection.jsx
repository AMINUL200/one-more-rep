...
const HandleHeroSection = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingHero) {
        ...
        response = await api.post(`/admin/banners/update/${editingHero.id}`, submitData, {
          headers: {
      ...
      if (response.data?.status) {
        toast.success(editingHero ? 'Hero section updated successfully' : 'Hero section added successfully');
        setShowModal(false);
        setEditingHero(null);
        setImagePreview(null);
        setFormData({
  ...
  const handleStatusToggle = async (hero) => {
    try {
      const response = await api.patch(`/admin/hero-sections/${hero.id}/status`, {
  ...
  const handleSortOrder = async (id, newOrder) => {
    try {
      const response = await api.patch(`/admin/hero-sections/${id}/sort-order`, {
        sort_order: newOrder
      });
      if (response.data?.status) {
            ...
            <div className="flex justify-between items-center mb-6">
              <div>
                ...
                <p className="text-sm mt-1" style={{ color: colors.textLight }}>
                  {editingHero ? `Editing: ${editingHero.title}` : 'Create a new hero banner'}
                </p>
              </div>
              <button
                onClick={() => {
                      ...
                      >
                        {imagePreview ? (
                          ...
                          <div>
                            ...
                            <p className="text-sm" style={{ color: colors.textLight }}>Click to upload hero image</p>
            ...
            <table className="w-full">
              <thead style={{ backgroundColor: colors.background }}>
                <tr style={{ borderBottom: `1px solid ${colors.border}` }}>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider" style={{ color: colors.textLight }}>Image</th>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider" style={{ color: colors.textLight }}>Buttons</th>
              ...
              <tbody>
                {filteredHeroList.length === 0 ? (
                  ...
                  filteredHeroList.map((hero) => (
                    <tr key={hero.id} style={{ borderBottom: `1px solid ${colors.border}` }}>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() => handleSortOrder(hero.id, hero.sort_order - 1)}
                            className="p-1 hover:bg-gray-100 rounded transition-colors"
                            disabled={hero.sort_order === 1}
                          >
                            <ArrowUp size={14} style={{ color: hero.sort_order === 1 ? colors.muted : colors.primary }} />
                          </button>
                          <span className="text-sm font-medium" style={{ color: colors.text }}>{hero.sort_order}</span>
                          <button
                            onClick={() => handleSortOrder(hero.id, hero.sort_order + 1)}
                            className="p-1 hover:bg-gray-100 rounded transition-colors"
                            disabled={hero.sort_order === heroList.length}
                          >
                            <ArrowDown size={14} style={{ color: hero.sort_order === heroList.length ? colors.muted : colors.primary }} />
                          </button>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap" style={{ color: colors.textLight }}>#{hero.id}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {hero.image ? (
                          ...
                          <div className="space-y-1">
                            {hero.button1_text && (
                              <div className="text-xs">
                                <span className="font-medium">1:</span> {hero.button1_text}
                              </div>
                            )}
                            {hero.button2_text && (
                              <div className="text-xs">
                                <span className="font-medium">2:</span> {hero.button2_text}
                              </div>
                            )}
                          </div>
                        ) : (
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        >
                          {(hero.status === 1 || hero.status === true) ? 'Active' : 'Inactive'}
                        </button>
                      </td>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-[60] bg-black/50">
            <div className="bg-white rounded-lg w-full max-w-md p-6 shadow-xl">
              ...
              <p className="mb-4" style={{ color: colors.textLight }}>
                Are you sure you want to delete hero section "{deleteConfirm.title}"? This action cannot be undone.
              </p>
